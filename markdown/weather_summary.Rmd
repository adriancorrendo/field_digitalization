...
summary_winter <- 
  historical.summary.DigitalAg %>% 
  # Filter months
  dplyr::filter(Month %in% c(10,11,12,1,2,3,4)) %>% 
  group_by(ID, Year) %>% 
  summarise(PP = sum(PP), 
            Tmean = mean(Tmean),
            ETE = sum(ETE),
...
plot_COMPLETE_anom_10 <- ggplot(data= anomalies_annual)+ 
...
plot_COMPLETE_EXTPP <- anomalies_annual %>% 
  group_by(ID) %>% 
  mutate(EPE_av = mean(EPE)) %>% ungroup() %>%
  mutate(paint = case_when(EPE > EPE_av ~ "yes",
                           TRUE ~ "no")) %>% 
  ggplot(aes(x=Year, y = EPE))+
  geom_col(aes(fill = paint))+
  scale_fill_manual(name = "", values = c("grey", "#118ab2"))+
  scale_x_continuous(limits = c(1981, 2021), breaks = seq(1982, 2022, by = 2))+
...
plot_summer_anom_10 <- ggplot(data= anomalies_summer)+ 
...
plot_summer_anom_extrem <- ggplot(data= anomalies_summer)+
geom_point(aes(x=T_anom, y = PP_anom), shape = 16, size = 1.25)+
geom_hline(yintercept = 0, linetype = "dashed")+
geom_vline(xintercept = 0, linetype = "dashed")+
    labs(x='Temp_Anom', y= 'Precip_Anom')+ 
...
plot_JulyAug_EXTTEMP <- anomalies_JulAug %>% 
  group_by(ID) %>% 
  mutate(ETE_av = mean(ETE)) %>% ungroup() %>% 
  ggplot(aes(x=Year, y = ETE))+
  geom_col()+
...
plot_winter_anom_10 <- ggplot(data= anomalies_winter)+ 
geom_point(aes(x=T_anom, y = PP_anom), shape = 16, size = 1.25)+
geom_hline(yintercept = 0, linetype = "dashed")+
geom_vline(xintercept = 0, linetype = "dashed")+
    labs(x='Temp_Anom', y= 'Precip_Anom')+
...
plot_winter_anom_extrem <- ggplot(data= anomalies_winter)+ 
geom_point(aes(x=T_anom, y = PP_anom), shape = 16, size = 1.25)+
geom_hline(yintercept = 0, linetype = "dashed")+
geom_vline(xintercept = 0, linetype = "dashed")+
    labs(x='Temp_Anom', y= 'Precip_Anom')+
...
plot_winter_EXTPP <-anomalies_winter %>% 
  group_by(ID) %>% 
  mutate(EPE_av = mean(EPE)) %>% ungroup() %>% 
  ggplot(aes(x=Year, y = EPE))+
  geom_col()+
